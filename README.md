...
- solo se mantiene un subconjunto de municipios (ver `SYNCED_MUNICIPALITIES`, `SYNCED_CAT_MUNICIPALITIES` y `SYNCED_GVA_MUNICIPALITIES` en `functions/src/services/official-sync.ts`).

**Sincronización:** los trabajos programados `syncOpenRta` (lunes 04:30 Europe/Madrid) y `syncCatalunya` (martes 04:30) y `syncValencia` (miércoles 04:30), todos en `europe-west1` porque Cloud Scheduler no opera en `europe-southwest1`, y el callable `adminSyncOfficialData` reconstruyen las colecciones `officialVut`, `officialStats`, `officialCells` y `officialCellPins` de forma diferencial (las celdas obsoletas se eliminan en cada pasada y la purga de bajas está acotada por fuente). Descargas iniciales: julio de 2026 (~50.900 viviendas en 10 municipios andaluces; ~10.700 en Barcelona; ~14.500 en València, Alicante y Benidorm).

**Sin respaldo oficial:** ninguna administración citada (Junta de Andalucía, Generalitat de Catalunya, Ajuntament de Barcelona, Generalitat Valenciana, Dirección General del Catastro) respalda, patrocina o avala Viviendas Perdidas. La cita de las fuentes es un crédito neutral y no implica relación, colaboración ni aprobación por parte de los titulares de los datos. Los datos oficiales se ofrecen «tal cual» y «según disponibilidad», sin garantías.
